...
test.describe('Form Layout Page', () => {
    ...
    test('input fields', async ({ page }) => {
        ...
        await usingTheGridEmailInput.pressSequentially('test@example.com', { delay: 100 })
...
test('tooltips', async ({ page }) => {
    ...
    page.getByRole('tooltip') // only if role tooltip was created
...
test('web tables', async ({ page }) => {
    ...
    // 2. Get the row based on the column value
    await page.locator('.ng2-smart-pagination-nav').getByText('2').click()
    const targetRowById = page.getByRole('row', { name: '11' }).filter({ has: page.locator('td').nth(1).getByText('11') })
    await targetRowById.locator('.nb-edit').click()
    await page.locator('input-editor').getByPlaceholder('E-mail').clear()
    await page.locator('input-editor').getByPlaceholder('E-mail').fill('test111@example.com')
    await page.locator('.nb-checkmark').click()
    await expect(targetRowById.locator('td').nth(5)).toHaveText('test111@example.com')
